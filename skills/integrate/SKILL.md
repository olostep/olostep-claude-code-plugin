...
Using the typed `LLMExtract` class (schema-based):
```python
from olostep import LLMExtract, Format
...
Using a prompt (via raw dict — the `prompt` field is only available via REST):
```python
result = client.scrapes.create(
...
If the user is also using Claude Code or Cursor, set up the MCP server config alongside the SDK integration.

**Claude Code** — add to `.claude/settings.json` or global Claude config:
```json
{
  "mcpServers": {
    "olostep": {
...
4. Suggest next steps:
   ...
   - "Check out `/olostep:scrape`, `/olostep:search`, and `/olostep:research` for ad-hoc web data"
